...
pub(super) const CACHE_TRACKS: &str = "tracks.db";

/// Cached albums database filename.
pub(super) const CACHE_ALBUMS: &str = "albums.db";

/// Cached artists database filename.
pub(super) const CACHE_ARTISTS: &str = "artists.db";

/// Cached playlists database filename.
pub(super) const CACHE_PLAYLISTS: &str = "playlists.db";

impl Library {
    /// Load cached items from the file at `cache_path` into the given `store`.
    pub(super) fn load_cache<T: DeserializeOwned>(&self, cache_path: &Path, store: &mut Vec<T>) {
    ...
    pub(super) fn save_cache<T: Serialize>(&self, cache_path: &Path, store: &[T]) {
    ...
    fn needs_download(&self, remote: &crate::model::playlist::Playlist) -> bool {
        self.playlists
            .read()
            .unwrap()
            .iter()
    ...
    fn append_or_update(&self, updated: crate::model::playlist::Playlist) -> usize {
    ...
    fn fetch_shows(&self) {
        ...
        let mut saved_shows: Vec<crate::model::show::Show> = Vec::new();
    ...
    pub(super) fn fetch_playlists(&self) {
        ...
        while let Some(lists) = lists_batch {
            for (index, remote) in lists.iter().enumerate() {
                ...
                if self.needs_download(remote) {
                    ...
                    let mut playlist: crate::model::playlist::Playlist = remote.clone();
    ...
    fn fetch_artists(&self) {
        let mut artists: Vec<crate::model::artist::Artist> = Vec::new();
    ...
    fn insert_artist(&self, id: String, name: String) {
        ...
        {
            let mut artist = crate::model::artist::Artist::new(id.to_string(), name.to_string());
    ...
    fn fetch_albums(&self) {
        let mut albums: Vec<crate::model::album::Album> = Vec::new();
        let mut i = 0u32;
    ...
    pub(super) fn populate_artists(&self) {
